...
describe('Agent Flow Integration Tests', () => {
  ...
  describe('Complete Agent Processing Flow', () => {
    test('should handle joke request end-to-end', async () => {
      ...
      const response = await agentService.processMessage(userMessage, [], undefined, undefined, 'user123');
  ...
  describe('Conversation Context Integration', () => {
    test('should initialize and maintain conversation context', async () => {
      ...
      const response = await agentService.processMessageWithConversation(userId, 'Tell me a joke');
      expect(response).toHaveProperty('content');
      // May fall back to general if OpenAI API fails in test environment
      expect(['joke', 'general']).toContain(response.agentUsed);
    ...
    test('should handle agent handoffs properly', async () => {
      ...
      const response1 = await agentService.processMessageWithConversation(userId, 'Tell me a joke');
      ...
      const response2 = await agentService.processMessageWithConversation(userId, 'Make me laugh');
    ...
    test('should track conversation satisfaction and performance', async () => {
      ...
      conversationManager.updateContext(userId, 'That was great! Thanks!', 'Glad you enjoyed it!', 'joke');
  ...
  describe('RAG Integration with Classification', () => {
    test('should return relevant content based on classification', async () => {
      const testCases = [
        { message: 'Make me laugh with a joke', expectedAgent: 'joke', expectedCategory: 'joke' },
        { message: 'Show me a reaction gif', expectedAgent: 'gif', expectedCategory: 'gif' },
        { message: 'Tell me something fascinating', expectedAgent: 'trivia', expectedCategory: 'trivia' }
      ];
      ...
      for (const testCase of testCases) {
        ...
        if (ragContent) {
          ...
          const validCategories = {
            ...
            'trivia': ['trivia', 'space', 'science', 'botany', 'nature', 'plants']
          };
          expect(validCategories[testCase.expectedCategory as keyof typeof validCategories] || [testCase.expectedCategory]).toContain(ragContent.category);
        }
      }
    });
    ...
    test('should handle fallback when no specific content found', async () => {
      ...
      expect(['general', 'website_support', 'joke', 'trivia', 'gif']).toContain(classification.agentType);
      
      // Should still get some content (fallback)
      const ragContent = ragService.searchForAgent('joke', obscureMessage, true);
  ...
  describe('Error Handling Integration', () => {
    test('should gracefully handle invalid agent types', async () => {
      ...
      const response = await agentService.processMessage('Hello', [], undefined, undefined, 'user123');
      expect(response).toHaveProperty('content');
      expect(response).toHaveProperty('agentUsed');
    ...
    test('should handle empty or invalid messages', async () => {
      ...
      for (const testMessage of testCases) {
        const response = await agentService.processMessage(testMessage, [], undefined, undefined, 'user123');
        expect(response).toHaveProperty('content');
        expect(response.content).toBeTruthy();
      }
    });
    ...
    test('should handle conversation management errors gracefully', async () => {
      ...
      const response = await agentService.processMessageWithConversation(userId, 'Hello');
      expect(response).toHaveProperty('content');
      expect(response.content).toBeTruthy();

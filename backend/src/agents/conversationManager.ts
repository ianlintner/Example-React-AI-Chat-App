...
export interface HandoffTrigger {
  type: 'explicit_request' | 'topic_shift' | 'performance_decline' | 'expertise_mismatch' | 'conversation_stagnation';
  confidence: number;
...
export class ConversationManager {
  ...
  initializeContext(userId: string, initialAgent: AgentType = 'general'): ConversationContext {
  ...
  updateContext(userId: string, userMessage: string, agentResponse: string, currentAgent: AgentType): ConversationContext {
    ...
    const handoffTrigger = this.detectHandoffTriggers(context, userMessage, currentAgent);
    if (handoffTrigger) {
  ...
  private analyzeMessage(context: ConversationContext, userMessage: string, agentResponse: string): void {
    const lowerMessage = userMessage.toLowerCase();
    
    // Update topic detection
    if (lowerMessage.includes('code') || lowerMessage.includes('programming') || lowerMessage.includes('bug')) {
    ...
    } else if (lowerMessage.includes('fact') || lowerMessage.includes('trivia') || lowerMessage.includes('learn')) {
      context.conversationTopic = 'educational';
    } else if (lowerMessage.includes('gif') || lowerMessage.includes('meme') || lowerMessage.includes('visual')) {
      context.conversationTopic = 'visual_entertainment';
    }

    // Analyze user satisfaction based on response patterns
    const positiveIndicators = ['thanks', 'great', 'awesome', 'perfect', 'love', 'amazing', 'helpful', 'good', 'nice'];
    const negativeIndicators = ['no', 'stop', 'boring', 'not helpful', 'wrong', 'bad', 'annoying', 'stupid'];
    
    const positiveCount = positiveIndicators.filter(word => lowerMessage.includes(word)).length;
    const negativeCount = negativeIndicators.filter(word => lowerMessage.includes(word)).length;
  ...
  private detectHandoffTriggers(context: ConversationContext, userMessage: string, currentAgent: AgentType): HandoffTrigger | null {
    ...
    if ((lowerMessage.includes('gif') || lowerMessage.includes('meme') || lowerMessage.includes('visual')) && currentAgent !== 'gif') {
      return {
        type: 'explicit_request',
        confidence: 0.8,
        targetAgent: 'gif',
  ...
  private detectTopicMismatch(context: ConversationContext, currentAgent: AgentType): HandoffTrigger | null {
    ...
    if (idealAgent && idealAgent !== currentAgent && context.conversationDepth > 2) {
      return {
  ...
  generateHandoffMessage(context: ConversationContext): string | null {
    ...
    const handoffMessages: Record<string, Record<AgentType, string>> = {
      ...
      'expertise_mismatch': {
        ...
        'general': "Our General Assistant might have a fresh perspective on this topic.",
        'account_support': "This appears to be account-related. Our Account Support Specialist would be better equipped to handle this securely and efficiently.",
      ...
      'performance_decline': {
        ...
        'general': "Let me bring in our General Assistant for a fresh approach to your question.",
        'account_support': "Let me connect you with our Account Support Specialist who might have better solutions for your account concerns.",
      ...
      'conversation_stagnation': {
        ...
        'general': "Our General Assistant might bring a fresh perspective to keep our conversation engaging.",
        'account_support': "Let me bring in our Account Support Specialist for a fresh approach to your account-related concerns.",
    ...
    const reasonCategory = context.handoffReason?.includes('explicit') ? 'explicit_request' :
                          context.handoffReason?.includes('expertise') ? 'expertise_mismatch' :
                          context.handoffReason?.includes('performance') ? 'performance_decline' :
                          'conversation_stagnation';

    return handoffMessages[reasonCategory]?.[context.handoffTarget] || 
  ...
  getHandoffInfo(userId: string): { target: AgentType; reason: string; message: string } | null {
    const context = this.contexts.get(userId);
    if (!context?.shouldHandoff || !context.handoffTarget) return null;
    ...
    return {
      ...
      message: this.generateHandoffMessage(context) || 'Switching to better agent'
    };
  }
